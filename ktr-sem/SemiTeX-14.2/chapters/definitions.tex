...
\begin{itemize}
	...
	\item Broad network access: Capabilities are available over the network accessed through standard mechanism that promote use by heterogeneous thin or thick client platforms (e.g., mobile phones, tablets, laptops, and workstations).
...
\end{itemize}(\cite[p. 2f]{CloudNIST11})


\subsection{Fog Computing}\label{sec:def_fog}

"Fog is an emergent architecture  for computing, storage, control and networking that distributed these services closer to the end-users along the cloud-to-things continuum". \cite[p. 854]{DBLP:journals/iotj/ChiangZ16}. However, Fog Computing is not a strict architecture that has a clear definition, "instead it represents a notion that supports to push data analyrics towards leaves(i.e. edge nodes)"\cite[p. 3]{Perera:2017:FCS:3101309.3057266}. Chiang and Perera et al. both stress that the important part of the definition is that tasks are not supposed to be completed by the edge nodes only, but on every node on the path to the cloud server as needed by the use case.



\image{8cm}{fog-and-cloud.png}{Fog-interfaces\cite[p. 862]{DBLP:journals/iotj/ChiangZ16}}{img:fog}
...
\image{9cm}{f2c.png}{Fog2Cloud Architecture\cite[p. 4]{DBLP:journals/corr/Marin-TorderaMA16}}{img:f2c}
